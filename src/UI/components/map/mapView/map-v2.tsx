...
const MapWrapperV3: React.FC<MapWrapperV3Props> = ({ doiResolverId }) => {
  ...
  useEffect(() => {
    // Only call API if "Hard" filters change (e.g. area coordinates, date ranges)
    // or if we have no data at all.
    const hasData = occurrenceData.length > 0;

    const debounceHandler = setTimeout(() => {
      // If you only want to hit the API for things WebGL CAN'T do (like area selection)
      // you would check specific sub-filters here.
      dispatch(getOccurrenceData(filters));
    }, 400);

    return () => clearTimeout(debounceHandler);
    // Remove 'filters' and watch specific hard-filter properties instead:
  }, [filters.areaCoordinates, filters.timeRange, dispatch]);
  /* ---------------- 2. Soft Filtering (Species Toggles) ---------------- */
  /* ---------------- 2. Soft Filtering (Species Toggles) ---------------- */
  /* ---------------- 2. Soft Filtering (GPU-driven) ---------------- */
  useEffect(() => {
    ...
    applyFiltersWebGL(presenceSource, {
      activeSpecies: selectedSpecies,
      showDetected,
      showNotDetected,
    });

    applyFiltersWebGL(absenceSource, {
      activeSpecies: selectedSpecies,
      showDetected,
      showNotDetected,
    });
    ...
    showDetected,
    showNotDetected,
    occurrenceData, // ensures re-run when new data loads
  ]);

  /* ---------------- 3. Data Processing (Only on Data Load) ---------------- */
  useEffect(() => {
    if (
    ...
    // Clear and add features only when the raw data from the API changes
