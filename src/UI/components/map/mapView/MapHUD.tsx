...
const MapHUD: React.FC<MapHUDProps> = ({
  ...
  const donutData = sortedFilteredSpecies.slice(0, 9).map(([sp, count]) => {
    if (sp === OTHER_LABEL)
      return { name: 'Other Anopheles', value: count, color: '#038543' };
    const style = speciesStyles.find((s) => normalize(s.species) === sp);
    ...
    <>
      ...
      {totalFilteredPoints > 0 && (
        <Box textAlign="center" mt={isExpanded ? -1 : 0.5}>
          <Typography fontSize={9} sx={{ opacity: 0.6 }}>
      ...
      {totalFilteredPoints > 0 && (
        <>
          ...
          >
            ...
            {/* Not detected card */}
            <Box
              onClick={() => setShowNotDetected((p) => !p)}
              role="button"
      ...
      {isExpanded && (
        <>
          ...
          <Box
            maxHeight={isMobile ? 200 : isTablet ? 180 : 220}
            overflow="auto"
            sx={{
              position: 'relative',
              '&::-webkit-scrollbar': { width: 3 },
              '&::-webkit-scrollbar-thumb': {
                background: 'rgba(126,239,168,0.25)',
                borderRadius: 4,
              },
            }}
          >
                ...
                <React.Fragment key={sp}>
                  <Box
                    ...
                    sx={{
                      ...
                      mb: 1,
                      p: isMobile ? 0.75 : isTablet ? 0.75 : 1,
                      borderRadius: 2,
                      overflow: 'hidden',
                  ...
                  >
                    ...
                    >
                      <Box display="flex" alignItems="center" gap={1}>
                        ...
                        />
                        <Typography
                          fontSize={isMobile ? 10 : isTablet ? 10 : 12}
                          fontWeight={700}
                          fontStyle="italic"
                          noWrap
                        >
      ...
      <>
        ...
        <Box
          sx={{
            ...
            borderBottom: 'none',
            boxShadow:
              '0 -6px 36px rgba(126,239,168,0.12), inset 0 0 20px rgba(0,0,0,0.5)',
            color: 'white',
            display: 'flex',
            flexDirection: 'column',
          }}
