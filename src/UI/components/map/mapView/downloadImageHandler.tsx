...
export const registerDownloadHandler = (
  ...
  function downloadHandler() {
    ...
    map.once('rendercomplete', async function () {
      ...
      const scale = width > 1920 ? 1.4 : 1.0;
      ...
      if (
        ...
        activeYear = speciesOrFilters.year ?? selectedYear ?? '';
      }
      ...
      // Grouping logic: Primary species + 'other species'
      ...
      activeSpeciesList.forEach((s) => {
        ...
        const clean = lower.replace(/^(anopheles|an\.)\s+/i, '');
        // Explicitly catch 'other anopheles' or 'other species'
        if (lower.includes('other anopheles') || lower === 'other species') {
      ...
      if (displaySpeciesList.length > 0) {
        ...
        displaySpeciesList.forEach((s: any, i: number) => {
          ...
          mapContext.arc(
            itemX + 3 * scale,
            itemY,
            Math.max(1.5 * scale, Math.min(2.8 * scale, itemHeight / 3.2)),
            0,
            2 * Math.PI
          );
          mapContext.fill();
