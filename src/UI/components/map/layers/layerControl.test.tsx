...
describe("LayerControl", () => {
  ...
  it('renders a layer button with the correct name', () => {
    render(<LayerControl name="testOverlay" displayName="Test overlay" isVisible={true} />, {
      map: state,
    });
  ...
  it('dispatches the correct action when checkbox is toggled', () => {
    const { store } = render(<LayerControl name="testOverlay" displayName="Test overlay" isVisible={true} />, {
      map: state,
    });
  ...
  it('renders a color button with the current layer colour when color control is closed', () => {
    ...
    expect(screen.getByRole('button', { name: 'current color'})).toHaveStyle(`background: rgba(255,0,0,1)`);
  })
