...
jest.mock('../../state/speciesInformation/actions/upsertSpeciesInfo.action', () => ({
  getSpeciesInformation: jest.fn((id) => ({ type: 'test-getSpeciesInformation', payload: id})),
  upsertSpeciesInformation: jest.fn((info) => ({ type: 'test-upsertSpeciesInformation', payload: info})),
}))

jest.mock('@mui/material/CircularProgress', () => () => (<div>Circular progress mock</div>))
...
describe('SpeciesInformationEditor', () => {
  ...
  it('starts with validation errors when creating', () => {
    ...
    expect(wrapper.getByText('Short description cannot be empty')).toBeInTheDocument();
    expect(wrapper.getByText('Create').closest('button')).toHaveAttribute('disabled');
  })
  ...
  it('dispatches an action to load species information if there is an id', () => {
    ...
    expect(actions[0]).toEqual({ type: 'test-getSpeciesInformation', payload: '123-456'})
  })
  ...
  it('shows progress spinner and disables fields when loading', () => {
    ...
    expect(wrapper.getByText('Update').closest('button')).toHaveAttribute('disabled');
  })
  ...
  it('populates the form correctly when species information is loaded', () => {
    ...
    expect(wrapper.queryByText('Circular progress mock')).not.toBeInTheDocument();
    ...
    expect(wrapper.getByText('test description', {exact: false})).toBeInTheDocument();
  })
  ...
  it('saves species information with the right details', () => {
    ...
    fireEvent.change(inputs[1], {target: {value: 'entered short description'}})
    ...
    expect(actions[0]).toEqual({ type: 'test-upsertSpeciesInformation', payload: expectedSpeciesInfo})
  })
  ...
  it('sets image when user uploads file', async () => {
    ...
    await waitFor(() => {
      ...
      expect(speciesImg).toEqual('data:application/octet-stream;base64,YWFhYWFhYWFhYWE=')
    });
  })
  ...
  it('displays an error if the file is above 512KB', async () => {
    ...
    fireEvent.change(input, {
      target: {
        files: [new File([...Array(512*1024)].map(_ => 'a'), 'test-file')]
      }
    });

    await waitFor(() => {
      expect(toast.error).toHaveBeenCalledWith('Uploaded files must be less than 512 KB.', {autoClose: 5000})
    });
  })
})