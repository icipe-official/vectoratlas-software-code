...
jest.mock('../components/shared/AuthWrapper', () => function MockAuthWrapper({role, children}: {role: string, children: any}) {
  return <div data-testid={`auth=${role}`}>{children}</div>;
});
jest.mock('../components/sources/source_form', () => function MockSourceForm() {
