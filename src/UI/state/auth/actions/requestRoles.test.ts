...
jest.mock('../../../api/queries', () => ({
  roleRequestMutation: jest.fn(
    (requestReason, rolesRequested, email) =>
      'requestReason: ' +
      requestReason +
      ', roles: ' +
      rolesRequested +
...
describe('requestRoles', () => {
  ...
  it('calls fetchGraphQlDataAuthenticated', async () => {
    await requestRoles({
      ...
      email: 'test@test',
    })(mockThunkAPI.dispatch, mockThunkAPI.getState, null);
  ...
  it('dispatches toast and action on success', async () => {
    ...
    expect(toast.success).toHaveBeenCalledWith('Role request submitted.');
  });
