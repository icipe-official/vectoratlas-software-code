  ...
  async (filters: MapState['filters'], thunkAPI) => {
    ...
    while (hasMore === true) {
      const anotherResponse = await fetchGraphQlData(
        occurrenceQuery(responseNumber, numberOfItemsPerResponse, singularOutputs(filters))
      );
      const moreSiteLocations = anotherResponse.data.OccurrenceData.items;
      thunkAPI.dispatch(
        updateOccurrence({ data: [...siteLocations, ...moreSiteLocations], searchID})
      );
...
export const { updateOccurrence, drawerToggle, drawerListToggle, layerToggle,
  filterHandler, startNewSearch, updateMapLayerColour } =
  mapSlice.actions;
