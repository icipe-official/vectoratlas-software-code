...
import { getSpeciesInformation, upsertSpeciesInformation } from "./upsertSpeciesInfo.action";
...
jest.mock('../../../api/queries', () => ({
  ...
  upsertSpeciesInformationMutation: jest.fn().mockImplementation(info => "upsertSpeciesInformationMutation: " + info.id),
}))
...
describe('species info actions', () => {
  ...
  it('getSpeciesInformation dispatches setCurrentInfoForEditing with the result from the API', async () => {
    ...
    expect(fetchGraphQlDataAuthenticated).toHaveBeenCalledWith("speciesInformationById: 123-456", "token12345")
    expect(mockThunkAPI.dispatch).toHaveBeenCalledWith(setCurrentInfoForEditing(expectedSpeciesInformation))
  });
  ...
  it('upsertSpeciesInformation calls the API to save information correctly', async () => {
    ...
    expect(fetchGraphQlDataAuthenticated).toHaveBeenCalledWith("upsertSpeciesInformationMutation: save-test-123", "token12345")
  })
})