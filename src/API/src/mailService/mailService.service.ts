...
export class MailService {
  ...
  async sendEmail(
    ...
    try {    
      ...
      const commLog = await this.saveLog(communicationLog,
        allRecipients,
        emailBody
      );
      //send email
  ...
  async saveLog(communicationLog: CommunicationLog, 
    recipients: Array<string>, message: string) : Promise<CommunicationLog> {
    if(communicationLog){
      this.communicationLogService.create(communicationLog);
    } else {
  ...
  async updateSentStatus (communicationLog: CommunicationLog, info:  SMTPTransport.SentMessageInfo) {
    if(info.messageId) {
