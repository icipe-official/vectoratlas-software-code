...
export class FixBrokenCitationYearUniqueContraintLogic1784294706153
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "reference" DROP CONSTRAINT IF EXISTS "UQ_30733720a40f8d4bb3b83d06735"`,
    );
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX "IDX_reference_citation_year_unique_both_set" ON "reference" (citation, "year") WHERE citation <> '' AND "year" IS NOT NULL`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_reference_citation_year_unique_both_set"`,
    );

    // Restore the old constraint on (citation, year) which was too strict (enforced even with empty citation)
    await queryRunner.query(
      `ALTER TABLE "reference" ADD CONSTRAINT "UQ_30733720a40f8d4bb3b83d06735" UNIQUE (citation, year)`,
    );
  }
}
