...
async getPointData(entityType: string, occurrenceId: string): Promise<any> {
  switch (entityType) {
    ...
    case 'Larval_site':
      return this.larvalSiteRepository.find({ where: { occurrence: { id: occurrenceId } } });

    case 'dataset':
      return this.datasetRepository.find({ where: { occurrence: { id: occurrenceId } } });

    case 'insecticideResistanceBioassays':
      return this.insecticideResistanceBioassaysRepository.find({ where: { occurrence: { id: occurrenceId } } });

    case 'bionomics':
      return this.bionomicsRepository.find({ where: { occurrence: { id: occurrenceId } } });
    ...
    case 'cytochromesP450_cypMethodAndSample': {
      const records = await this.cytochromesP450CypMethodAndSampleRepository.find({
        where: {
          insecticideResistanceBioassays: {
            occurrence: { id: occurrenceId },
          },
    ...
    case 'recordedSpecies':
      return this.speciesRepository.find({ where: { occurrence: { id: occurrenceId } } });

    case 'reference':
      return this.referenceRepository.find({ where: { occurrence: { id: occurrenceId } } });

    case 'sample':
      return this.sampleRepository.find({ where: { occurrence: { id: occurrenceId } } });

    case 'site':
      return this.siteRepository.find({ where: { occurrence: { id: occurrenceId } } });
