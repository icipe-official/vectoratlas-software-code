...
describe("Occurrence service", () => {
  ...
  it("findOneById finds one by ID from the repository", async () => {
    ...
    occurrenceRepositoryMock.findOne = jest.fn().mockResolvedValue(expectedOccurrence);
    
    const result = await service.findOneById("123")
    expect(result).toEqual(expectedOccurrence)
    expect(occurrenceRepositoryMock.findOne).toHaveBeenCalledWith({"where": {"id": "123"}})
  })

  it("findAll returns all samples", async () => {
    const expectedOccurrences = [new Occurrence(), new Occurrence(), new Occurrence()]
    occurrenceRepositoryMock.find = jest.fn().mockResolvedValue(expectedOccurrences);
